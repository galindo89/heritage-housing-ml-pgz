...
def model_performance_body():
    ...
    During training, the target variable `SalePrice` was log-transformed using `log10` to reduce right skew and improve model fit. 
    This transformation was reversed after prediction to obtain sale prices in their original scale.
    ...
    st.markdown("""
    - The model was trained using all features available in the training dataset, including those with strong correlation to `SalePrice`.  
    As a result, the input data must include all expected variables to ensure correct pipeline execution and accurate predictions.  
    This may be revised in future versions with further feature reduction and pipeline refactoring.

    - The current design assumes the full feature set is passed through the feature engineering pipeline, which includes encoding, imputation, transformation, and correlation filtering. Skipping or manually removing features before prediction will result in pipeline errors or degraded performance.
    """)
